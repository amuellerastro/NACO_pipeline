...
pro batch_NACO_reduce_Science_noAGPM_Mband, star, path, filestar, onlymed, fwhm, qintsky, display

restore, filestar, /verbose
ra_st = ra
dec_st = dec

restore, path+'../Reduced/sky_ident.sav', /v
...
idx = where(catg eq 'SCIENCE' and naxis1 ge 256)
rawf = files[idx]
...
for qu=0,step-1 do begin
  ...
  ;sky background
  skf = file_search(path+'../Reduced/'+'sky_'+strcompress(qu+1,/rem)+'_background_*fits', count=nsk)
  ...
  ;     endif
	...
	flag1024 = 0
      ...
      find, mtmp, fdx, fdy, flux, sharp, roundness, 2*stddev(mtmp), 5., [-1,1], [0.2,1.0]
...
;      tyc = idx0[1]
  ...
  ;       endfor
	...
	mtmp = corim[512-384:512+383,512-384:512+383]
	flag1024 = 1
    ...
    sim = dblarr(nframes)
    if (n_elements(corim[0,0,*]) gt 1) then medim = median(corim, dim=3) else medim = corim
  ;   for j=0,nframes-1 do sim[j] = stddev(corim[*,*,j]-medim)
    for j=0,nframes-1 do sim[j] = stddev(corim[nx/2.-120:nx/2.+121,nx/2.-120:nx/2.+121,j]-medim)
...
;       weights = 1./sqrt(weights)
  ...
  endfor	;loop over files of each quadrant

endfor	;loop over the quadrants
...
writefits, path+'../Reduced/'+'img_Mp_dc.fits', redim, hdrsciref
writefits, path+'../Reduced/'+'vec_Mp_paral.fits', pa
writefits, path+'../Reduced/'+'vec_Mp_paral.fits', ha, /append
