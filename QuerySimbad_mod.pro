PRO QuerySimbad_mod, name, ra, de, id, Found = found, NED = ned, ERRMSG = errmsg, $
    Verbose = verbose, CFA=cfa, Server=server, SILENT=silent, $
    Print = print,Vmag=Vmag,Jmag=Jmag,Hmag=Hmag,Kmag=Kmag,f12=f12,pmra=pmra,pmde=pmde, $
    epmra=epmra,epmde=epmde,parallax=parallax, eparallax=eparallax, spt=spt, rv=rv, $
...
; look for J2000 coords
  ...
  if cnt GE 1 then begin
      ...
      ; Get IRAS F12 flux if present
      f12i = where(strpos(Result, '%O.IRAS ') ne -1,f12cnt)
      if f12cnt GE 1 then f12 = float(strmid(result[f12i], 40, 6))

      ; Get proper motion if present
      pmi = where(strpos(Result, '%P ') ne -1,pmcnt)
      if pmcnt GE 1 then begin 
				reads,strmid(Result[pmi],2),pmra

				;to extract PM DEC more effort needed
				tmp = strmid(result[pmi], 3, strlen(result[pmi]))	;remove leading space
				pos = strpos(tmp, ' ')
				tmp2 = strmid(tmp, pos, strlen(tmp))
				reads,strmid(tmp2,0),pmde
      ...
      ;get error of PM
      if (pmra ne 0. and pmde ne 0.) then begin

				pos1 = strpos(result[pmi], '[')
				pos2 = strpos(result[pmi], ']')
				tmp = (strmid(result[pmi], pos1+1, pos2-pos1-1))[0]
				pos1 = strpos(tmp, ' ')
				pos2 = strpos(tmp, ' ', /reverse_search)

				epmra = strmid(tmp, 0, pos1)
				epmde = strmid(tmp, pos1+1, pos2-pos1-1)
      ...
      if plxcnt GE 1 then reads,strmid(Result[plxi],2),parallax else parallax = !values.f_nan

      ;extract error of plx
      if (finite(parallax) eq 1) then begin
				pos1 = strpos(result[plxi], '[')
				pos2 = strpos(result[plxi], ']')
				eparallax = strmid(result[plxi], pos1+1, pos2-pos1-1)
      endif else begin
				eparallax = !values.f_nan
      endelse

      ; Get SpT if present
      spt = ''
      spti = where(strpos(Result, '%S ') ne -1,sptcnt)
      if sptcnt GE 1 then reads,strmid(Result[spti],2),spt
      if sptcnt GE 1 then spt = strcompress(strmid(Result[spti],2,4),/rem) else spt = ''

      ; Get RV if present
      rvi = where(strpos(Result, '%V v ') ne -1,rvcnt)
      if rvcnt GE 1 then rv = float(strmid(Result[rvi],4,6)) else rv = !values.f_nan
