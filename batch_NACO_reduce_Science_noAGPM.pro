...
pro batch_NACO_reduce_Science_noAGPM, star, path, filestar, onlymed, fwhm, qintsky, display
...
;       corimdk[*,*,j] = outimdk
  ...
  ;     endif
    ...
    find, mtmp, fdx, fdy, flux, sharp, roundness, 3*stddev(mtmp), 10., [-1,1], [0.2,1.0]
...
;     for j=0,nframes-1 do sim[j] = stddev(corim[nx/2.-174:nx/2.+175,nx/2.-174:nx/2.+175,j]-medim)
    for j=0,nframes-1 do sim[j] = stddev(corim[round(txc)-10:round(txc)+9,round(tyc)-10:round(tyc)+9,j]-medim[round(txc)-10:round(txc)+9,round(tyc)-10:round(tyc)+9])
